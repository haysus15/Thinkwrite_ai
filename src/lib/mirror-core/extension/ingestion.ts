...
import type { VoiceFingerprint as InternalVoiceFingerprint } from "@/lib/mirror-core/voiceAnalysis";
...
export async function ingestExtensionFingerprint(params: {
  ...
  const { error: chamberError } = await supabase.from("voice_chambers").upsert(
    {
      user_id: userId,
      chamber,
  ...
  const { error: cacheError } = await supabase.from("voice_profiles").upsert(
    {
      user_id: userId,
      aggregate_fingerprint: nextOverall.aggregateFingerprint,
      confidence_level: nextOverall.confidenceLevel,
  ...
  if (cacheError) {
    console.warn("[Mirror Extension Ingestion] voice_profiles cache update failed:", cacheError.message);
  }
