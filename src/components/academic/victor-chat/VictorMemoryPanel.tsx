...
export default function VictorMemoryPanel({
  ...
  const t = useTranslations("academic.victorUi.memory");
  ...
  const grouped = useMemo(() => {
    ...
    for (const item of items) {
      const key = item.class_name || t("uncategorized");
  ...
  const handleResolve = useCallback(async (id: string) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data?.error || t("errors.resolve"));
      }
      setItems((prev) =>
        prev.map((item) =>
          item.id === id
      ...
      setError(resolveError instanceof Error ? resolveError.message : t("errors.resolve"));
    } finally {
      setUpdatingId(null);
    }
  }, []);
  ...
  const handleClearAll = useCallback(async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data?.error || t("errors.clear"));
      }
      setItems([]);
    } catch (clearError) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-3xl rounded-2xl border border-white/10 bg-[#0B1220] p-4 text-slate-100 shadow-2xl">
        ...
        <div className="mt-4 max-h-[70vh] overflow-y-auto pr-1">
          {loading ? (
            <AcademicLoadingState message={t("loading")} className="!min-h-0 py-6" />
          ) : error ? (
                    ...
                    {entries.map((entry) => (
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          <p>
                            {t("struggledWith")} <span className="font-medium">{entry.concept}</span>
                          </p>
                          {entry.resolved ? (
                            <span className="text-[11px] text-emerald-300">{t("resolved")}</span>
                          ) : (
                            <button
                              type="button"
                            ...
                            >
                              {updatingId === entry.id ? t("saving") : t("markResolved")}
                            </button>
                          )}
                        </div>
                        <p className="mt-1 text-[11px] text-slate-400">
                          {t("detected")} {new Date(entry.detected_at).toLocaleDateString()}
                        </p>
                      </div>
                    ))}
                  </div>
