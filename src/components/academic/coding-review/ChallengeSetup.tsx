...
export default function ChallengeSetup({
  ...
  const t = useTranslations("academic.codeReviewMode.setup");
  ...
  const handleConfirmChallenges = async (challenges: ParsedCodeChallenge[]) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data?.error || t("errors.saveChallenges"));
      onCreated(setId);
    } catch (confirmError) {
      setError(
        confirmError instanceof Error ? confirmError.message : t("errors.confirmChallenges")
      );
    } finally {
      setIsBusy(false);
    }
  ...
  const handleManualStart = async () => {
    ...
    try {
      ...
      if (challenges.length === 0) {
        throw new Error(t("errors.addOneChallenge"));
      }
      const response = await fetch("/api/code-review/challenge/confirm", {
        method: "POST",
        headers: { "content-type": "application/json" },
      ...
      setError(
        manualError instanceof Error ? manualError.message : t("errors.saveManualChallenges")
      );
    } finally {
      setIsBusy(false);
    }
    ...
    <section className="space-y-4 rounded-2xl border border-white/10 bg-slate-900/50 p-4">
      ...
      <div className="grid gap-2 md:grid-cols-2">
        ...
        <input
          ...
          placeholder={t("placeholders.challengeCount")}
          inputMode="numeric"
          className="rounded border border-white/20 bg-slate-950/30 p-2 text-sm text-slate-100"
        />
      </div>
      ...
      {method === "paste" && !parsedChallenges && (
        <div className="space-y-2">
          ...
          <button
            ...
            className="rounded-full border border-sky-300/40 bg-sky-500/15 px-3 py-1.5 text-xs text-sky-100 disabled:opacity-60"
          >
            {isBusy ? t("parsing") : t("parseChallenges")}
          </button>
        </div>
      )}
      ...
      {method === "upload" && !parsedChallenges && (
        <div className="space-y-2">
          ...
          <button
            ...
            className="rounded-full border border-sky-300/40 bg-sky-500/15 px-3 py-1.5 text-xs text-sky-100 disabled:opacity-60"
          >
            {isBusy ? t("parsing") : t("parseChallengeSet")}
          </button>
        </div>
      )}
        ...
        <div className="space-y-2">
          {manualRows.map((row, index) => (
            <input
              ...
              placeholder={t("challengeNumber", { index: index + 1 })}
              className="w-full rounded border border-white/20 bg-slate-950/30 p-2 text-sm text-slate-100"
            />
          ))}
