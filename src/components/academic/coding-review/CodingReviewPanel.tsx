// src/components/academic/coding-review/CodingReviewPanel.tsx
"use client";
...
export default function CodingReviewPanel({
  ...
  setContextId = null,
  assignmentId: assignmentIdProp = null,
}: {
  initialReviewId?: string | null;
  setContextId?: string | null;
  assignmentId?: string | null;
}) {
  const t = useTranslations("academic.codeReviewMode.panel");
  const { profile } = useAuth();
  const router = useRouter();
  const searchParams = useSearchParams();
  const assignmentId = assignmentIdProp ?? searchParams.get("assignmentId");
  ...
  useEffect(() => {
    ...
    const loadReview = async () => {
      try {
        ...
        if (!active) return;
        setCompletionError(
          loadError instanceof Error ? loadError.message : t("errors.loadReview")
        );
      }
    };
    void loadReview();
  ...
  const handleMarkComplete = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data?.error || t("errors.completeChallenge"));
      }
      setSessionMeta((prev) =>
        prev
      ...
      setCompletionError(
        markError instanceof Error ? markError.message : t("errors.completeChallenge")
      );
    } finally {
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data?.error || t("errors.unlockChallenge"));
      }
      setSessionMeta((prev) =>
        prev
      ...
      setCompletionError(
        unlockError instanceof Error ? unlockError.message : t("errors.unlockChallenge")
      );
    } finally {
      setCompletionBusy(false);
    }
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/10 bg-slate-900/40 px-4 py-2">
        <div className="flex flex-wrap items-center gap-2">
          {setContext && sessionMeta?.set_order ? (
            ...
            >
              {setContext.title} → {t("challengeNumber", { number: sessionMeta.set_order })}
            </button>
          ) : null}
          {setContext?.assignment_prompt ? (
            <details className="rounded border border-white/15 bg-white/5 px-2 py-1 text-[11px] text-slate-300">
              <summary className="cursor-pointer">{t("challengeContext")}</summary>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {!isComplete ? (
            ...
            >
              {completionBusy ? t("saving") : t("markComplete")}
            </button>
          ) : (
            <button
              type="button"
            ...
            >
              {completionBusy ? t("unlocking") : t("unlockToEdit")}
            </button>
          )}
        </div>
      </div>
