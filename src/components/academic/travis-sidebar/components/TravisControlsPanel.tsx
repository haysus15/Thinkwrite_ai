...
export default function TravisControlsPanel({
  ...
  const t = useTranslations("academic.travisUi.controls");
  ...
  return (
    <details className={`${shared.card} ${shared.cardPad}`}>
      ...
      <div className="mt-3 space-y-2 text-sm text-slate-300">
          ...
          <div className="space-y-2 rounded-xl border border-teal-400/30 bg-teal-500/10 p-3">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {creatingAssignment ? t("saving") : t("saveAssignment")}
              </button>
              <button
                type="button"
                onClick={() => {
          ...
          <div className="space-y-2 rounded-xl border border-teal-400/30 bg-teal-500/10 p-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                ...
                <option value="custom">{t("customCadence")}</option>
              </select>
              <select
              ...
              >
                {WEEKDAYS.map((day) => (
                  <option key={day} value={day}>{t("dueOn", { day })}</option>
                ))}
              </select>
            </div>
            <textarea
            ...
            {classPlans.map((plan) => (
              ...
              >
                <span className="truncate pr-2">
                  {plan.class_name} · {plan.cadence === "weekly" ? t("weeklyShort", { day: plan.due_weekday }) : t("customShort")}
                </span>
                <button
                  type="button"
        ...
        {uploading && <AcademicLoadingState message={t("parsingSyllabus")} className="!min-h-0 py-4" />}

        {parsedSyllabusId && (
          <div className="space-y-2 rounded-xl border border-teal-400/30 bg-teal-500/10 p-3">
            <StatusBadge status={t("pendingReview")} />
                ...
                <div key={draft.id} className="rounded-lg border border-white/15 bg-black/20 p-2">
                  <input
                    ...
                    className="w-full rounded border border-white/15 bg-black/25 px-2 py-1 text-xs text-slate-100 focus:border-teal-300 focus:outline-none"
                      placeholder={t("reviewAssignmentName")}
                  />
            ...
            >
              {publishing ? t("publishing") : t("approvePublish", { count: approvedCount })}
