...
export default function AssignmentSetup({
  ...
  const t = useTranslations("academic.paperWorkflow.setup");
  ...
  const ensureSet = async () => {
    ...
    if (!response.ok) throw new Error(data?.error || t("errors.createSet"));
  ...
  const runParse = async (selectedMethod: Extract<Method, "paste" | "upload">) => {
    ...
    try {
      ...
      if (selectedMethod === "paste") {
        ...
        const data = await response.json();
        if (!response.ok) throw new Error(data?.error || t("errors.parseText"));
        setParsedPrompts(Array.isArray(data?.prompts) ? data.prompts : []);
      } else {
        if (!uploadFile) throw new Error(t("errors.chooseFile"));
        ...
        const data = await response.json();
        if (!response.ok) throw new Error(data?.error || t("errors.parseUpload"));
        setParsedPrompts(Array.isArray(data?.prompts) ? data.prompts : []);
      }
    } catch (parseError) {
  ...
  const handleConfirmPrompts = async (prompts: ParsedAssignmentPrompt[]) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data?.error || t("errors.savePrompts"));
      onCreated(setId);
    } catch (confirmError) {
      setError(
        confirmError instanceof Error ? confirmError.message : t("errors.confirmPrompts")
      );
    } finally {
      setIsBusy(false);
    }
  ...
  const handleManualStart = async () => {
    ...
    try {
      ...
      if (prompts.length === 0) {
        throw new Error(t("errors.addOnePrompt"));
      }
      const response = await fetch("/api/paper/assignment/confirm", {
        method: "POST",
        headers: { "content-type": "application/json" },
      ...
      setError(
        manualError instanceof Error ? manualError.message : t("errors.saveManualPrompts")
      );
    } finally {
      setIsBusy(false);
    }
    ...
    <section className="space-y-4 rounded-2xl border border-white/10 bg-slate-900/50 p-4">
      ...
      <div className="grid gap-2 md:grid-cols-2">
        ...
        <textarea
          ...
          placeholder={t("placeholders.assignmentPrompt")}
          rows={3}
        ...
        <textarea
          ...
          placeholder={t("placeholders.rubric")}
          rows={3}
          className="rounded border border-white/20 bg-slate-950/30 p-2 text-sm text-slate-100 md:col-span-2"
        />
        <input
          ...
          placeholder={t("placeholders.paperCount")}
          inputMode="numeric"
          className="rounded border border-white/20 bg-slate-950/30 p-2 text-sm text-slate-100"
        />
      </div>
      ...
      {method === "paste" && !parsedPrompts && (
        <div className="space-y-2">
          <textarea
            ...
            rows={8}
            placeholder={t("placeholders.pasteText")}
          ...
          <button
            ...
            className="rounded-full border border-sky-300/40 bg-sky-500/15 px-3 py-1.5 text-xs text-sky-100 disabled:opacity-60"
          >
            {isBusy ? t("parsing") : t("parsePrompts")}
          </button>
        </div>
      )}
      ...
      {method === "upload" && !parsedPrompts && (
        <div className="space-y-2">
          ...
          <button
            ...
            className="rounded-full border border-sky-300/40 bg-sky-500/15 px-3 py-1.5 text-xs text-sky-100 disabled:opacity-60"
          >
            {isBusy ? t("parsing") : t("parseAssignment")}
          </button>
        </div>
      )}
        ...
        <div className="space-y-2">
          {manualRows.map((row, index) => (
            <input
              ...
              placeholder={t("promptNumber", { index: index + 1 })}
              className="w-full rounded border border-white/20 bg-slate-950/30 p-2 text-sm text-slate-100"
            />
          ))}
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="rounded-full border border-sky-300/40 bg-sky-500/15 px-3 py-1.5 text-xs text-sky-100 disabled:opacity-60"
            >
              {isBusy ? t("saving") : t("startWriting")}
            </button>
          </div>
        </div>
      )}
