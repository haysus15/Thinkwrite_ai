...
export default function MathStepEditor({
  ...
  const primaryAction = (() => {
    if (step.status === "correct" || step.status === "equivalent_form" || step.status === "likely_correct") {
      return {
        label: t("academic.mathMode.stepEditor.addNextStep"),
        onClick: () => onAddStep(),
    ...
    if (step.status === "incorrect" || step.status === "error" || step.status === "partial") {
      return {
        label: t("academic.mathMode.stepEditor.editStep"),
        onClick: () => {
    ...
    return {
      label: step.status === "needs_recheck"
        ? t("academic.mathMode.stepEditor.reverify")
        : t("academic.mathMode.stepEditor.verify"),
  ...
  const primaryLabel =
    isVerifying &&
    (step.status === "unchecked" || step.status === "needs_recheck")
      ? t("academic.mathMode.stepEditor.verifying")
      : primaryAction.label;
  ...
  const secondaryAction = (() => {
    ...
    if (step.status === "unchecked") {
      return { label: t("academic.mathMode.stepEditor.requestHint"), onClick: () => onHint?.(), disabled: !onHint };
    }
    if (step.status === "equivalent_form") {
      return {
        label: t("academic.mathMode.stepEditor.seeNote"),
    ...
    if (step.status === "likely_correct") {
      return {
        label: t("academic.mathMode.stepEditor.flagForReview"),
    ...
    <div className="group border-b border-white/10 pb-4">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 text-sm text-slate-300">
          <span className="text-sm font-semibold text-slate-100">{t("academic.math.stepLabel")} {stepNumber}</span>
        ...
        <div className="flex items-center gap-2">
          ...
          {!isEditing && (
            <button
              ...
              title={t("academic.mathMode.stepEditor.editStep")}
            >
          ...
          >
            {t("academic.mathMode.stepEditor.askVictor")}
          </button>
          <button
            type="button"
      ...
      <textarea
        ...
        placeholder={t("academic.mathMode.stepEditor.reasoningPlaceholder")}
        rows={2}
        disabled={Boolean(isWorkspaceLocked)}
      ...
      {canMarkFinalAnswer && (
        <div className="mt-2">
          ...
          >
            {isCompletingSession
              ? t("academic.mathMode.stepEditor.completing")
              : step.is_final_answer
              ? t("academic.mathMode.stepEditor.finalAnswerMarked")
              : t("academic.mathMode.stepEditor.markFinalAnswer")}
          </button>
        </div>
      )}
    </div>
