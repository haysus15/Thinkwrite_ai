...
export default function ParseReview({
  ...
  return (
    <section className="space-y-3 rounded-xl border border-white/10 bg-slate-900/40 p-4">
      <header>
        <h3 className="text-sm font-medium text-slate-100">{t("academic.mathMode.parseReview.title")}</h3>
        <p className="mt-1 text-xs text-slate-400">
          {t("academic.mathMode.parseReview.subtitle")}
        </p>
      </header>
          ...
          <div key={`${problem.order}-${index}`} className="rounded-lg border border-white/10 p-2">
            <div className="mb-1 flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1">
                ...
                >
                  {t("academic.mathMode.parseReview.up")}
                </button>
                <button
                  type="button"
                  onClick={() =>
                  ...
                  className="rounded border border-white/20 px-1.5 py-0.5 text-[11px] text-slate-200"
                >
                  {t("academic.mathMode.parseReview.down")}
                </button>
                <button
                  type="button"
                  onClick={() =>
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        >
          {isSaving ? t("academic.entry.saving") : t("academic.mathMode.parseReview.confirm")}
        </button>
      </div>
    </section>
  );
