    ...
    <aside className={styles.teacher}>
      <header className={styles.header}>
        <div className={styles.presenceRow}>
          ...
          <span className="text-[11px] text-slate-400">Teacher zone</span>
        </div>
      ...
      <footer className={styles.verification}>
        <div className={styles.resultList}>
          {meaningfulSteps.length === 0 ? (
            ...
            <span>{pendingCount} step{pendingCount === 1 ? "" : "s"} pending verification.</span>
          )}
        </div>
        <button
          type="button"
        ...
        >
          {isVerifying
            ? "Checking..."
            : pendingCount > 0
            ? `Check my work (${pendingCount} pending)`
            : "Check my work"}
        </button>
      </footer>
    </aside>
