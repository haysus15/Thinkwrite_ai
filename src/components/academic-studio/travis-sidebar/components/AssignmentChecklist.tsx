...
export default function AssignmentChecklist({
  ...
  const toggleHistory = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to load assignment history.");
      }
  ...
  return (
    <>
      ...
      {assignment.agenda_date && (
        <p className={`mt-1 text-xs ${overdue ? "text-red-100/80" : "text-slate-400"}`}>
          Planned: {formatDueDate(assignment.agenda_date)}
        </p>
      )}
      ...
      {assignment.is_at_risk && (
        <p className="mt-1 text-xs text-amber-200">At risk: needs attention this week</p>
      )}
      {prepProgress && (
        <p className={`mt-1 text-xs ${overdue ? "text-red-100/80" : "text-slate-400"}`}>
          Prep progress: day {prepProgress.elapsed} of {prepProgress.totalDays}
        </p>
      )}
      {guidancePreview && (
        <p className={`mt-1 text-xs ${overdue ? "text-red-100/80" : "text-slate-400"}`}>
