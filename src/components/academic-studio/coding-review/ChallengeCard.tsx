...
export default function ChallengeCard({
  ...
  const description = challengeDescription(session.victor_context);
  const type = challengeType(session.victor_context);
  const hasCode = String(session.code_snapshot || "").trim().length > 0;
  const status = session.is_complete ? "Complete" : hasCode ? "In progress" : "Not started";
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-slate-100">Challenge {session.set_order || "-"}</p>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px] text-slate-400">
        ...
        <span>{session.language || "Language not set"}</span>
      </div>
    </button>
  );
}
