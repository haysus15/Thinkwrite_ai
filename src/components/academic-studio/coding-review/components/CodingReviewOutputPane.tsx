      ...
      <div className="flex-1 overflow-y-auto bg-slate-950/30 px-4 py-3 text-xs text-slate-100">
        ...
        {assistLoading && (
          <AcademicLoadingState
            message="Victor is generating a guided breakdown..."
        ...
        {assistResponse && !assistLoading && (
          <div className="mb-3 rounded-md border border-sky-400/30 bg-sky-500/10 px-3 py-2 text-slate-100">
            <p className="text-[10px] uppercase tracking-[0.2em] text-sky-200">Guided assistance</p>
        ...
        {!error && !output && (
          <AcademicEmptyState
            title="No output yet"
            description="Run your code to see output or errors."
          ...
          <div className="space-y-3">
            {output.error ? (
              ...
              <div className="overflow-x-auto">
                ...
                <p className="mt-2 text-[10px] text-slate-400">{output.rowCount} rows</p>
              </div>
            )}
          </div>
        )}
        ...
        {output?.type !== "sql" && output && (
          <div className="space-y-3">
            ...
            {!output.stdout && !output.stderr && !output.error && (
              <AcademicEmptyState
                title="No output"
                description="This run returned no stdout, stderr, or error output."
                className="!min-h-0 py-3"
              />
            )}
          </div>
