...
export default function TravisReminderPanel({
  ...
  const loadActive = useCallback(async () => {
    ...
    if (!activeResponse.ok) {
      throw new Error(activeData?.error || "Could not load Travis reminders.");
    }
    if (activeResponse.ok) {
  ...
  const loadAtRiskCounts = useCallback(async () => {
    ...
    if (!response.ok) {
      throw new Error(data?.error || "Could not load assignment risk data.");
    }
  ...
  const evaluateAndLoad = useCallback(async () => {
    ...
    try {
      ...
      if (!evaluateResponse.ok) {
        throw new Error(evaluateData?.error || "Could not evaluate Travis reminders.");
      }
      await loadActive();
      await loadAtRiskCounts();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Could not load Travis reminders.");
    } finally {
      setLoading(false);
    }
  ...
  if (mode === "floating" && collapsed) {
    return (
      <div className="fixed bottom-5 right-5 z-40">
        ...
        >
          <span className="font-semibold tracking-wide text-teal-200">Travis</span>
  ...
  return (
    <div className={containerClass}>
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-teal-200">Travis reminders</p>
      ...
      <div className="mt-2">
        <p className="text-xs text-slate-300">
          You have {sortedReminders.length} active reminder{sortedReminders.length === 1 ? "" : "s"}.
      ...
      <div className="mt-2 space-y-2">
        {isViewingCurrentWeek && currentWeekAtRiskCount > 0 && (
          <p className="rounded-lg border border-amber-400/20 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
            You have {currentWeekAtRiskCount} at-risk item{currentWeekAtRiskCount === 1 ? "" : "s"} this week.
          </p>
        )}
        {!isViewingCurrentWeek && viewedWeekAtRiskCount === 0 && currentWeekAtRiskCount > 0 && (
          <p className="rounded-lg border border-amber-400/20 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
            Nothing at risk in the week you are viewing. {currentWeekAtRiskCount} item{currentWeekAtRiskCount === 1 ? "" : "s"} at risk this week.
          </p>
        )}
