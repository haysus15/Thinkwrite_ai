      ...
      <div className="h-full w-full max-w-xl overflow-y-auto border-l border-white/10 bg-slate-950 p-5">
        ...
        <div className="mt-4 space-y-4">
          <div className="rounded-lg border border-white/10 bg-white/[0.03] p-3">
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-slate-400">Details</p>
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.03] p-3">
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-slate-400">Tasks</p>
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.03] p-3">
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-slate-400">Change history</p>
            {historyLoading ? (
              <p className="mt-2 text-xs text-slate-400">Loading history...</p>
            ) : historyRows.length === 0 ? (
              <p className="mt-2 text-xs text-slate-400">No recent changes.</p>
            ) : (
                ...
                {historyRows.map((row) => {
                  ...
                  return (
                    <p key={row.id} className="text-xs text-slate-300">
                      {oldStatus && newStatus
                        ? `Status changed from ${titleCaseStatus(oldStatus)} to ${titleCaseStatus(newStatus)}`
                        : row.change_type}
                      {" · "}
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.03] p-3">
            {selectedAssignment.syllabus_id ? (
              ...
              >
                From: {selectedAssignment.class_name} syllabus
              </button>
            ) : (
              <p className="text-xs text-slate-400">Added manually</p>
            )}
            <button
              type="button"
              onClick={() =>
