...
export default function AssignmentsWorkspace() {
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Could not load assignments. Try refreshing.");
      }
      setRows((data.assignments || []) as AssignmentListRow[]);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Could not load assignments. Try refreshing."
      );
    } finally {
      setLoading(false);
    }
  ...
  const fetchHistory = useCallback(async (assignmentId: string) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || "Failed to load change history.");
      }
  ...
  const saveDetailPanel = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to save assignment details.");
    } finally {
  ...
  const activeFilterChips: FilterChip[] = [
    ...
    statusFilter.length
      ? {
          key: "status",
          label: `Status: ${statusFilter.map(titleCaseStatus).join(", ")}`,
    ...
    priorityFilter.length
      ? {
          key: "priority",
          label: `Priority: ${priorityFilter.join(", ")}`,
          onClear: () => setPriorityFilter([]),
        }
      : null,
    dueRange !== "all"
      ? { key: "dueRange", label: `Due: this ${dueRange}`, onClear: () => setDueRange("all") }
      : null,
    searchText.trim()
      ? { key: "search", label: `Search: ${searchText.trim()}`, onClear: () => setSearchText("") }
      : null,
  ...
  return (
    <div className={`${shared.root} ${shared.page} space-y-5`}>
      <div className={shared.surfacePanel}>
        <p className={shared.panelTitle}>Assignments</p>
        <p className={shared.panelBody}>
          Canonical assignment records for status, priority, and planning handoff.
        </p>
      </div>
      ...
      {loading ? (
        <div className={shared.surfacePanel}>
          <AcademicLoadingState message="Loading assignments..." className="!min-h-0 py-4" />
        </div>
      ) : groups.length === 0 || filteredRows.length === 0 ? (
        <div className={shared.surfacePanel}>
          <AcademicEmptyState
            title="No assignments match this filter"
            description="Clear one filter and try again."
