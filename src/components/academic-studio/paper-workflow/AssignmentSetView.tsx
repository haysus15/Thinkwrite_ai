...
import PaperSetCompletionPanel from "./PaperSetCompletionPanel";
import type { PaperSetSummary } from "./PaperSetSummaryPanel";
...
export default function AssignmentSetView({ setId }: { setId: string }) {
  const router = useRouter();
  const [setData, setSetData] = useState<AssignmentSet | null>(null);
  const [papers, setPapers] = useState<PaperItem[]>([]);
  const [setSummary, setSetSummary] = useState<PaperSetSummary | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [isSaving, setIsSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const completedCount = useMemo(
    () => papers.filter((paper) => paper.is_complete).length,
    [papers]
  );
  const allComplete = papers.length > 0 && completedCount === papers.length;
  const progressPercent = papers.length === 0 ? 0 : Math.round((completedCount / papers.length) * 100);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`/api/paper/assignment-set?id=${setId}&include=papers`);
        const payload = await response.json();
        if (!response.ok) {
          throw new Error(payload?.error || "Unable to load assignment set.");
        }
        if (!active) return;
        setSetData((payload?.set as AssignmentSet) || null);
        const incomingPapers = Array.isArray(payload?.papers)
          ? (payload.papers as PaperItem[])
          : [];
        setPapers(
          incomingPapers.sort(
        ...
        if (!active) return;
        setError(loadError instanceof Error ? loadError.message : "Unable to load assignment set.");
      } finally {
  ...
  useEffect(() => {
    ...
    const loadSummary = async () => {
      ...
      try {
        const response = await fetch("/api/paper/set-summary", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ assignment_set_id: setId }),
        });
        const data = await response.json();
        if (!response.ok) {
          throw new Error(data?.error || "Unable to load set summary.");
        }
        if (active) setSetSummary(data as PaperSetSummary);
      } catch (summaryError) {
        if (active) {
          setError(
  ...
  const handleSaveSetContext = async (updates: Record<string, unknown>) => {
    ...
    try {
      ...
      setSetData(data.set as AssignmentSet);
    } catch (saveError) {
      setError(
  ...
  if (isLoading) {
    return <p className="text-sm text-slate-300">Loading assignment set...</p>;
  }

  if (!setData) {
    return <p className="text-sm text-rose-200">Assignment set not found.</p>;
  }
  ...
  return (
    <div className="space-y-4">
      <header className="space-y-3 rounded-xl border border-white/10 bg-slate-900/40 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            {setData.class_name && <p className="text-xs text-slate-400">{setData.class_name}</p>}
          </div>
          <button
            type="button"
        ...
        {setData.rubric_text && (
          <details className="rounded border border-white/10 p-3">
            <summary className="cursor-pointer text-xs text-slate-300">View rubric</summary>
            <p className="mt-2 text-xs text-slate-300">{setData.rubric_text}</p>
          </details>
        )}

        <div>
          <p className="text-xs text-slate-300">
            {completedCount} of {papers.length} papers complete
          </p>
        ...
        <details className="rounded border border-white/10 p-3">
          ...
          <div className="mt-3 grid gap-2 md:grid-cols-2">
            ...
            />
            <textarea
              defaultValue={setData.rubric_text || ""}
              disabled={allComplete}
              onBlur={(event) =>
                event.target.value !== (setData.rubric_text || "") &&
                void handleSaveSetContext({ rubric_text: event.target.value })
              }
              rows={3}
              className="rounded border border-white/20 bg-slate-950/30 p-2 text-sm text-slate-100 md:col-span-2"
            />
      ...
      {!allComplete && (
        <div className="flex items-center justify-end">
          <button
            type="button"
            onClick={() => {
              const target = papers.find((paper) => !paper.is_complete) || papers[0];
              if (!target) return;
              router.push(`/academic/paper-workflow/paper/${target.id}?setId=${setId}`);
            }}
      ...
      {allComplete && setSummary && (
        <PaperSetCompletionPanel
          summary={setSummary}
          papers={papers}
          onVictorDebrief={() => {
            const target = papers[0];
            if (!target) return;
            router.push(`/academic/paper-workflow/paper/${target.id}?setId=${setId}&debrief=set`);
          }}
        />
      )}
      ...
      <div className="grid gap-2">
        {papers.map((paper) => (
          <PaperCard
            ...
            paper={paper}
            onOpen={(selected) =>
              router.push(`/academic/paper-workflow/paper/${selected.id}?setId=${setId}`)
            }
          />
        ))}
      </div>
