...
export default function PaperWorkflowHome() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!setRes.ok) {
          throw new Error(setData?.error || "Unable to load assignment sets.");
        }
        if (!papersRes.ok) {
          throw new Error(paperData?.error || "Unable to load papers.");
        }
        if (!active) return;
        ...
        if (!active) return;
        setError(
          loadError instanceof Error ? loadError.message : "Unable to load paper workflow."
        );
      } finally {
  ...
  if (loading) {
    return <p className="text-sm text-slate-300">Loading paper workflow...</p>;
  }

  return (
    <div className="space-y-4">
      <section className="rounded-xl border border-white/10 bg-slate-900/40 p-4">
        <h2 className="text-lg font-medium text-slate-100">Paper Workflow</h2>
      ...
      <section className="space-y-2 rounded-xl border border-white/10 bg-slate-900/40 p-4">
        ...
        {activeSets.length === 0 ? (
          <p className="text-xs text-slate-400">No active assignment sets yet.</p>
        ) : (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-[11px] text-slate-400">
                      {progress.complete} of {progress.total || set.paper_count || 0} complete
                    </span>
                  </div>
                  {set.class_name && (
