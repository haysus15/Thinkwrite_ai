...
export default function PromptParseReview({
  initialPrompts,
  onConfirm,
  onBack,
  isSaving,
}: {
  initialPrompts: ParsedAssignmentPrompt[];
  onConfirm: (prompts: ParsedAssignmentPrompt[]) => void;
  onBack: () => void;
  isSaving?: boolean;
}) {
  const [prompts, setPrompts] = useState<ParsedAssignmentPrompt[]>(initialPrompts);
  ...
  return (
    <section className="space-y-3 rounded-xl border border-white/10 bg-slate-900/40 p-4">
      <header>
        <h3 className="text-sm font-medium text-slate-100">Review parsed prompts</h3>
        <p className="mt-1 text-xs text-slate-400">
          Edit, delete, reorder, or add missing prompts before saving.
        </p>
      </header>

      <div className="space-y-2">
        {normalized.map((prompt, index) => (
          <div key={`${prompt.order}-${index}`} className="rounded-lg border border-white/10 p-2">
            <div className="mb-1 flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  type="button"
                  onClick={() => setPrompts((prev) => prev.filter((_, idx) => idx !== index))}
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        >
          {isSaving ? "Saving..." : "Confirm prompts"}
        </button>
      </div>
    </section>
  );
